...
function App() {
  ...
  useEffect(() => {
    ...
    if (dailyRecords.steps > 0 || heartRate > 0) {
        saveData();
    }
  ...
  const connectToDevice = async (device: Device) => {
    try {
      ...
      // STABLE MONITORING: Prevents UI lockup from high-frequency updates
      connected.monitorCharacteristicForService(HEART_RATE_SERVICE, HEART_RATE_CHAR, (err, char) => {
        if (char?.value) {
          try {
            ...
            // Validation: Only update state if BPM is valid to avoid chart crashes
            if (bpm > 30 && bpm < 220) { 
               setHeartRate(bpm);
      ...
      // Poll steps every 10s to keep the data stream consistent but lightweight
        ...
        <View style={styles.historySection}>
          ...
          <View style={styles.historyRow}>
            <View style={styles.historyCard}>
              <Text style={styles.historyLabel}>Saved Steps</Text>

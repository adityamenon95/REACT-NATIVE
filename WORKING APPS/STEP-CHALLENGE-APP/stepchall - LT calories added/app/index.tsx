...
function App() {
  ...
  // UPDATED: State for detailed daily records
  ...
  const connectToDevice = async (device: Device) => {
    try {
      ...
      // Heart Rate Monitoring with Crash Prevention
      ...
      // Steps Interval with Crash Prevention and Metric Calculation
        ...
        try {
          ...
          if (char?.value) {
            ...
            if (data.length >= 4) {
              ...
              // Calculations
              const dist = parseFloat(((count * 0.762) / 1000).toFixed(2)); // Avg stride 0.762m
              const cal = Math.floor(count * 0.04); // Approx 0.04 calories per step
...
const styles = StyleSheet.create({
  ...
  scanBtn: { backgroundColor: '#6C5CE7', paddingHorizontal: 15, paddingVertical: 10, borderRadius: 10 },

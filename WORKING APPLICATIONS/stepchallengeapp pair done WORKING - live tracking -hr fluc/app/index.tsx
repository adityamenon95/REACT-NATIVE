...
// --- EXPANDED UUIDS FOR NOISE (PULSE GO BUZZ) & KRONOS ---
...
// Added 0000fee7 which is common for Noise Pulse Go Buzz series
...
function App() {
  ...
  // --- NEW STATES FOR WEEKLY & FLUCTUATIONS ---
  ...
  // --- MONDAY RESET LOGIC ---
  const checkAndResetWeekly = useCallback(async (currentSteps: number, savedDate: string) => {
    const now = new Date();
    const dayOfWeek = now.getDay(); // 0 is Sunday, 1 is Monday
    const todayStr = now.toDateString();

    // Check if it's Monday (1) and if we haven't reset yet today
    if (dayOfWeek === 1 && savedDate !== todayStr) {
      console.log("Monday detected: Resetting weekly stats to zero.");
      setWeeklyStats({ steps: 0, calories: 0, distance: 0 });
  ...
  const loadData = async () => {
    try {
      ...
      if (jsonValue != null) {
        ...
        setLastResetDate(savedData.lastResetDate || "");
        
        // Check for Monday reset before applying weekly stats
        const didReset = await checkAndResetWeekly(savedData.dailyRecords?.steps || 0, savedData.lastResetDate || "");
        
        if (!didReset) {
            setWeeklyStats(savedData.weeklyStats || { steps: 0, calories: 0, distance: 0 });
        }
      }
  ...
  const updateWeeklyStats = (currentSteps: number) => {
    // Note: To make this truly additive, we track the delta or the weekly growth
    // For this implementation, it increments based on daily progress
    setWeeklyStats(prev => {
        const newSteps = prev.steps + 1; // Simulation of incremental growth or map to actual delta
        return {
            steps: currentSteps > prev.steps ? currentSteps : prev.steps, // Ensure it reflects the current week's highest sync
            calories: Math.floor((currentSteps > prev.steps ? currentSteps : prev.steps) * 0.045),
            distance: parseFloat((((currentSteps > prev.steps ? currentSteps : prev.steps) * 0.762) / 1000).toFixed(2))
        };
    });
  };
  ...
  const handleManualSave = async () => {
    try {
      const dataToSave = JSON.stringify({ dailyRecords, hrHistory, hrFluctuations, weeklyStats, lastResetDate });
  ...
  }, [dailyRecords, hrHistory, hrFluctuations, weeklyStats, lastResetDate]);
  ...
  // --- BLUETOOTH LOGIC (NOISE PULSE GO BUZZ COMPATIBLE) ---
  const startScan = () => {
    ...
    manager.startDeviceScan(null, null, (error, device) => {
      ...
      // Specific check for Noise devices in naming if available
      if (device?.name) {
  ...
  const connectToDevice = async (device: Device) => {
    try {
      ...
      // Noise Pulse Go Buzz sometimes requires MTU negotiation
      const connected = await device.connect();
      if (Platform.OS === 'android') {
        await connected.requestMTU(247); // Higher MTU for faster Noise data sync
      }
      ...
      connected.monitorCharacteristicForService(HEART_RATE_SERVICE, HEART_RATE_CHAR, (err, char) => {
        if (char?.value) {
          try {
            ...
            if (bpm > 30 && bpm < 220) { 
               // UPDATED: Faster capture for both graphs
               const timestamp = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' });
               setHeartRate(bpm);
               setHrHistory(prev => [...prev.slice(-9), bpm]); // Updates Red Graph
               setHrFluctuations(prev => [...prev.slice(-10), { value: bpm, time: timestamp }]); // Updates Purple Graph
      ...
      // Unified Step Interval (Noise/Kronos specific logic)
      const stepInterval = setInterval(async () => {
        try {
          ...
          if (char?.value) {
            const data = Buffer.from(char.value, "base64");
            // Noise data parsing can vary; standard is 4-byte buffer starting at offset 1 or 0
            const count = data.length >= 4 ? (data.readUInt32LE(data.length === 4 ? 0 : 1) & 0x00FFFFFF) : 0;
            if (count > 0) {
                ...
                // Ensure reset logic is checked during sync
                await checkAndResetWeekly(count, lastResetDate);
      ...
      setStatus("Failed");
      Alert.alert("Error", `Could not connect to ${device.name}. Ensure it's not connected to another app.`);
    }
  };
  ...
  }, [fetchGoogleFitSteps, checkAndResetWeekly]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{padding: 20}}>
        ...
        {/* --- WEEKLY SUMMARY (RESETS EVERY MONDAY) --- */}
        <Text style={styles.sectionTitle}>Weekly Summary (Mon Reset)</Text>
        ...
        {connectedDevice ? (
          <>
            ...
            <View style={[styles.card, {borderLeftColor: '#6C5CE7', borderLeftWidth: 5}]}>
              <Text style={styles.label}>HR FLUCTUATIONS (LOGGED BY TIME)</Text>
              {hrFluctuations.length > 0 ? (
                <LineChart 
                    data={{ 
                ...
                <Text style={{color: '#888', fontStyle: 'italic', marginTop: 10}}>Monitoring live data...</Text>
              )}
            </View>
          </>
        ) : (

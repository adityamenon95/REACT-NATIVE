...
function App() {
  ...
  const connectToDevice = async (device: Device) => {
    try {
      ...
      // FIX: Throttled HR Monitoring to prevent UI Crashing
      connected.monitorCharacteristicForService(HEART_RATE_SERVICE, HEART_RATE_CHAR, (err, char) => {
        if (char?.value) {
          try {
            ...
            let bpm = 0;
            if (data.length >= 2) {
               bpm = data[0] === 0x16 ? (data.length > 2 ? data.readUInt16LE(1) : data[1]) : data[1];
            }
            
            if (bpm > 30 && bpm < 220) { // Sanity check for HR values
               setHeartRate(prevBpm => {
                  // Only update if the value changed to reduce render cycles
                  if(prevBpm !== bpm) {
                    setHrHistory(prevHistory => [...prevHistory.slice(-9), bpm]);
                    setDailyRecords(prevRec => ({ ...prevRec, avgHr: bpm }));
                    return bpm;
                  }
                  return prevBpm;
               });
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{padding: 20}}>
        ...
        <View style={[styles.card, {borderLeftColor: '#4285F4', borderLeftWidth: 5}]}>
          <Text style={styles.label}>GOOGLE FIT (SENSORS)</Text>
          <Text style={styles.val}>{googleSteps.toLocaleString()}</Text>
        </View>

        {connectedDevice ? (
          <>
            <Text style={styles.sectionTitle}>Live Band Tracking</Text>
            <View style={[styles.card, {borderLeftColor: '#00CEC9', borderLeftWidth: 5, backgroundColor: '#E0F7F6'}]}>
              <Text style={styles.label}>LIVE STEPS (FROM BAND)</Text>
              <Text style={styles.val}>{bandSteps.toLocaleString()}</Text>
              <Text style={styles.liveIndicator}>• Live Syncing...</Text>
            </View>

            <View style={[styles.card, {borderLeftColor: '#FF2D55', borderLeftWidth: 5}]}>
              <Text style={styles.label}>LIVE HEART RATE</Text>
              <Text style={styles.val}>{heartRate} <Text style={{fontSize: 16}}>BPM</Text></Text>
              {/* Added fallback for Chart data to prevent crash if array is empty */}
              <LineChart 
                data={{ datasets: [{ data: hrHistory.length > 0 ? hrHistory : [0,0,0,0,0] }] }} 
        ...
        <View style={styles.historySection}>
          <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15}}>
            <Text style={styles.sectionTitle}>Stored Summary</Text>
          ...
          <View style={styles.historyRow}>
            <View style={styles.historyCard}>
              <Text style={styles.historyLabel}>Total Steps</Text>
